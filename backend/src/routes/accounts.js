...
module.exports = function accountRoutes(pool) {
    ...
    // GET /api/accounts
    router.get("/", asyncHandler("GET /accounts", async (req, res) => {
    ...
    // POST /api/accounts
    router.post("/", asyncHandler("POST /accounts", async (req, res) => {
        const a = req.body;
        const { rows } = await pool.query(
    ...
    // PUT /api/accounts/:id
    router.put("/:id", asyncHandler("PUT /accounts", async (req, res) => {
        ...
        const { rows } = await pool.query(
            `UPDATE accounts SET ${setClauses.join(", ")} WHERE id = $1 AND user_id = $${nextParamIndex} RETURNING *`,
            values
        );
    ...
    router.delete("/:id", asyncHandler("DELETE /accounts", async (req, res) => {
    ...
    router.post("/:id/pay-cc", asyncHandler("POST /accounts/:id/pay-cc", async (req, res) => {
    ...
    router.post("/:id/adjust-balance", asyncHandler("POST /accounts/:id/adjust-balance", async (req, res) => {
        const accountId = req.params.id;
